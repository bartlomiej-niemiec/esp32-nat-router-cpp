...
void WifiManager::OnMessage(const MessageQueue::Message & msg)
{
    ...
    if (dec.newState)
    {
        ...
        ESP_LOGI("WifiManager", "Changing to state %s", WifiNatRouter::
WifiNatRouterHelpers::WifiNatRouterStaToString(m_WifiManagerContext.m_WifiManagerState).data());
    }
    ...
    /// Check if new configuration is pending and start up nat router with pending config
...
{
    /// Check if new configuration is pending and start up nat router with pending config
    if ((s.mgrState == WifiNatRouterState::STOPPING || s.mgrState == WifiNatRouterState::NEW_CONFIGURATION_PENDING) &&
...
bool WifiManager::Startup(const WifiNatRouterConfig & config)
{
    MessageQueue::Message msg(MessageQueue::EventType::StartReq, -1);
    m_StartUpInProgress = true;
...
bool WifiManager::UpdateConfig(const WifiNatRouterConfig & config)
{
...
bool WifiManager::RegisterListener(EventListener * pEventListener)
{
    ...
    m_WifiNatRouterStateListeners.push_back(pEventListener);
    return true;
}
...
void WifiManager::RetryConnectToNetwork(void *arg)
{
    ...
    ESP_LOGI("WifiNatRouter", "Timer expired attempt: %i", pWifiManager->m_ReconnectCounterVal);
    MessageQueue::Message msg(MessageQueue::EventType::StaTimerReconnect, -1);
    pWifiManager->m_MessageQueue.Add(msg);
}
...
WifiNatRouterState WifiManager::GetState() const
{
...
{
    ESP_LOGI("WifiNatRouter", "Snapshot:");
    ESP_LOGI("WifiNatRouter", "WifiManagerState=%s", WifiNatRouter::
WifiNatRouterHelpers::WifiNatRouterStaToString(s.mgrState).data());
    ESP_LOGI("WifiNatRouter", "ApState=%i", static_cast<int>(s.apState));
    ESP_LOGI("WifiNatRouter", "staState=%i", static_cast<int>(s.staState));
    ESP_LOGI("WifiNatRouter", "scannerState=%s", getScannerStateString(s.scannerState).data());
    ESP_LOGI("WifiNatRouter", "scanningActive=%s", s.scanningActive ? "True" : "False");
    ESP_LOGI("WifiNatRouter", "updateConfig=%s", s.updateConfig ? "True" : "False");
    ESP_LOGI("WifiNatRouter", "startUpInProgress=%s", s.startUpInProgress ? "True" : "False");
    ESP_LOGI("WifiNatRouter", "staCfgValid=%s", s.staCfgValid ? "True" : "False");
    ESP_LOGI("WifiNatRouter", "reconnectCounterVal=%i", s.reconnectCounterVal);
}
...
WifiManager::Decision WifiManager::reduce(const WifiManager::Snapshot& s, const MessageQueue::Message& msg) const
{
    ...
    switch (msg.event)
    {
        ...
        {
            d.next = WifiNatRouterState::STARTED;
            d.newState = true;
        ...
        {
            d.next = WifiNatRouterState::STOPPING;
            d.newState = true;
        ...
        {
            d.next = WifiNatRouterState::NEW_CONFIGURATION_PENDING;
            d.newState = true;
            push(d, Effect::DisableNat);
            push(d, Effect::WifiStop);
            push(d, Effect::NotifyListener);
        ...
        {
            if (s.mgrState == WifiNatRouterState::STA_CANNOT_CONNECT)
            {
        ...
        case MessageQueue::EventType::EspWifiEvent:
        {
            switch (msg.espEventCode) {
                case WIFI_EVENT_AP_START:
                    ...
                    {
                        d.next = WifiNatRouterState::RUNNING;
                        d.newState = true;
                        push(d, Effect::NotifyListener);
                    }
                    break;
                case WIFI_EVENT_AP_STADISCONNECTED:
                    ...
                    break;  
                case WIFI_EVENT_STA_START:
                    if (s.staCfgValid && s.mgrState !=  WifiNatRouterState::STA_CANNOT_CONNECT)
                    {
                        d.next = WifiNatRouterState::CONNECTING;
                        d.newState = true;
                        push(d, Effect::StaConnect);
                        push(d, Effect::StartStaBackoffTimer);
                        push(d, Effect::NotifyListener);
        ...
        case MessageQueue::EventType::EspIpEvent:
        {
            switch (msg.espEventCode) {
                case IP_EVENT_STA_GOT_IP:
                    ...
                    push(d, Effect::StopStaBackoffTimer);
                    push(d, Effect::NotifyListener);
                    ESP_LOGI("WifiNatRouter", "Connected to AP");
                    break;
                case IP_EVENT_STA_LOST_IP:
                    if (s.mgrState == WifiNatRouterState::RUNNING)
                    {
                        d.next = WifiNatRouterState::CONNECTING;
                        d.newState = true;
                        push(d, Effect::DisableNat);
                        push(d, Effect::StartStaBackoffTimer);
                        push(d, Effect::NotifyListener);
                        ESP_LOGI("WifiNatRouter", "Disconnected from AP");
                    }
                    break;
            }
        }
        ...
        case MessageQueue::EventType::StaTimerReconnect:
        {
            if (!s.updateConfig && !s.scanningActive) {
                ...
                else if (s.reconnectCounterVal == RECONNECT_COUNTER_ATTEMPTS_COUNT)
                {
                    if (s.mgrState != WifiNatRouterState::STA_CANNOT_CONNECT)
                    {
                        d.next = WifiNatRouterState::STA_CANNOT_CONNECT;
                        d.newState = true;
                        push(d, Effect::NotifyListener);
                    }
                }
        ...
        {
            d.next = WifiNatRouterState::STOPPED;
            d.newState = true;
...
void WifiManager::runEffect(Effect e, const MessageQueue::Message& m)
{
    switch (e) {
        ...
        for (auto & listener : m_WifiNatRouterStateListeners)
        {
...
{
    const WifiNatRouterState state = GetState();
    return (false == m_ScanningActive && false == m_ShutdownInProgress) &&
        ...
        state == WifiNatRouterState::STARTED);
}
...
bool WifiManager::IsStartupPossible() const
{
    return (false == m_WifiManagerContext.m_PendingNewConfiguration &&
            ...
            (GetState() == WifiNatRouterState::STOPPED);
}

bool WifiManager::IsUpdateConfigPossible() const
{
   return  (GetState() >= WifiNatRouterState::CONNECTING) &&
